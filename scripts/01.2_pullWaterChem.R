...
chemicalphysical<-
  # read.csv(here("data/site_chemicalphysical_epi.csv")) %>%
  read.csv(here("LAGOS-ts-westernUS/data/site_chemicalphysical_epi.csv")) %>%
...
claritycarbon<-
  # read.csv(here("data/site_claritycarbon_epi.csv"))%>%
  read.csv(here("LAGOS-ts-westernUS/data/site_claritycarbon_epi.csv"))%>%
  mutate(event_date=lubridate::ymd(event_date),
         year=lubridate::year(event_date),
         year=factor(year),
         lagoslakeid=factor(lagoslakeid))
...
nutrientsalgae<-
  # read.csv(here("data/site_nutrientsalgae_epi.csv"))%>%
  read.csv(here("LAGOS-ts-westernUS/data/site_nutrientsalgae_epi.csv"))%>%
  mutate(event_date=lubridate::ymd(event_date),
         year=lubridate::year(event_date),
         year=factor(year),
         lagoslakeid=factor(lagoslakeid))
...
reservoir<-
  # read.csv(here("data/LAGOSUS_RSVR_v1.1.csv")) %>%
  read.csv(here("LAGOS-ts-westernUS/data/LAGOSUS_RSVR_v1.1.csv")) %>%
